...
export default class OrderManager{
    static textToOrder(transcript: string, dinnerList: IDinner[], styleList: IStyle[], detailList: IDetailedMenuTypeList, userID:string, phone:string, address1:string, address2:string, time:string):IOrder[] {
      let ret:IOrder[]=[];
      const demotranscript = "발렌타인 디너 심플 스타일에 빵은 빼고 커피는 다섯개";
      ...
      const detailNameList:string[] = Object.keys(detailList).map(key=>detailList[key].label);
      const detailDictionary:{[index:string]:string} = {};
      detailNameList.forEach(label=>detailList[label]?detailDictionary[label]=detailList[label].name:1);
      const words=demotranscript.split(" ");
      let from = 0, to = words.length;
      console.log(dinnerDictionary, styleDictionary, detailDictionary);
      while(from!=to){
        let tmpOrder:IOrder={};
        //디너이름 찾기 & 주문 범위 설정
            ...
            if(dinnerNameList[dnName]!=""&&words[i].includes(dinnerNameList[dnName])){
                if(gotDinner){
                    ...
                    Object.keys(dinnerDictionary[words[i]]).forEach(key=>{
                        tmpOrder[key]=dinnerDictionary[words[i]][key];
                    })
        ...
        for(let i = from;i<to;i++){
            ...
            if(stName!=""&&words[i].includes(stName)){
                if(!gotStyle){
                    gotStyle=true;
                    Object.keys(styleDictionary[words[i]]).forEach(key=>{
                        tmpOrder[key]=styleDictionary[words[i]][key];
                    })
                }
            }});
        }
        ...
        for(let i = from;i<to;i++){ //모든 단어에 대해 
            for(let dtName in detailNameList){ //디테일 이름 검사하는데
                ...
                if(dtFound && tmpOrder[detailDictionary[dtName]]){
                    let qt=-1;
                    ...
                    if(qt!=-1) tmpOrder[detailDictionary[dtName]]=qt;
                }
            }
        }
