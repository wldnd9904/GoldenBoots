...
app.post("/voucher", function (req, res) {
  const userID = req.body.userID;
  connection.query(
    "select * from voucheruserrelation where userID=?",
    [userID],
    function (err, rows) {
      //console.log(rows.map(data=>data.voucherID).toString())
      connection.query(
        `select * from voucher where voucherID IN (${rows
          .map((data) => data.voucherID)
          .toString()})`,
        [],
        function (err, rows) {
          //console.log(rows)
...
app.post("/vouchergrant", function (req, res) {
  ...
  const userID = req.body.userID;
  connection.query(
    "insert into voucheruserrelation values (?,?)",
    [voucherID, userID],
    function (err, rows) {
...
app.post("/voucheruse", function (req, res) {
  ...
  connection.query(
    "select * from voucheruserrelation where voucherID=? and userID=?",
    [voucherID, userID],
    function (err, rows) {
      if (rows.length) {
        connection.query(
          "delete from voucheruserrelation where voucherID=? and userID=?",
          [voucherID, userID],
          function (err, rows) {
            //if(err) throw err;
  ...
  connection.query(
    ...
    function (err, rows) {
      if (rows.length) {
        ...
        connection.query(
          "Update customer set? where userID=?",
          [sql_update, userID],
          function (err, rows) {
            //if(err) throw err;
          }
...
app.post("/order", function (req, res) {
  ...
  if (req.body.orderList) {
    connection.query(
      "select max(orderID) as max_ID from ordering",
      [],
      function (err, rows) {
        i = rows[0].max_ID;
        console.log("orderID: " + parseInt(i + 1));
        for (idx in req.body.orderList) {
          connection.query(
            "insert into ordering (orderID," +
              Object.keys(orderList[idx]).toString() +
              ") values (" +
              ++i +
              "," +
              `"` +
              Object.values(orderList[idx]).toString().replaceAll(",", `","`) +
              `"` +
              ")",
            [],
            function (err, rows) {
              //if(err) throw err;
              connection.query(
                "update ordering set description='pending' where orderID=?",
                [i],
                function (err, rows) {
                  //if(err) throw err;
                }
...
app.post("/recentorder", function (req, res) {
  const userID = req.body.userID;
  connection.query(
    'select *  from ordering where userID=? and description <> "canceled" Limit 10',
    [userID],
    function (err, rows) {
...
app.post("/ordercancel", function (req, res) {
  ...
  connection.query(
    "select * from ordering where orderID=?",
    [orderID],
    function (err, rows) {
      if (rows.length) {
        connection.query(
          'Update ordering set description="canceled" where orderID=?',
          [orderID],
          function (err, rows) {
            //if(err) throw err;
  ...
  if (req.body.order) {
    connection.query(
      ...
      function (err, rows) {
        //if(err) throw err;
        connection.query(
          "delete from ordering where orderID=?",
          [orderID],
          function (err, rows) {
            //if(err) throw err;
            connection.query(
              "insert into ordering (" +
                Object.keys(order).toString() +
                ") values (" +
                `"` +
                Object.values(order).toString().replaceAll(",", `","`) +
                `"` +
                ")",
              [],
              function (err, rows) {
...
app.post("/stockset", function (req, res) {
  ...
  connection.query(
    ...
    function (err, rows) {
      //if(err) throw err;
      connection.query(
        "update stock set stock = ? where name=?",
        [stock, name],
        function (err, rows) {
          //if(err) throw err;
      ...
      connection.query(
        "update detailedmenutype set price = ? where name=?",
        [price, name],
        function (err, rows) {
          //if(err) throw err;
...
app.post("/stockuse", function (req, res) {
  ...
  if (data != undefined && data.length) {
    for (pair in data) {
      ...
      connection.query(
        "select stock-? as remain from stock where name=?",
        [count, name],
        function (err, rows) {
          //if(err) throw err;
...
app.post("/styledelete", function (req, res) {
  ...
  connection.query(
    "select * from style where styleID=?",
    [styleID],
    function (err, rows) {
      if (rows.length) {
        connection.query(
          "delete from style where styleID=?",
          [styleID],
          function (err, rows) {
            //if(err) throw err;
...
app.post("/styleadd", function (req, res) {
  ...
  connection.query(
    "select max(styleID) as max_ID from style",
    [],
    function (err, rows) {
      //if(err) throw err;
      m = rows[0].max_ID;
      ...
      connection.query(
        "insert into style (styleID) values (?)",
        [m + 1],
        function (err, rows) {
          //if(err) throw err;
          res.json(rows);
          ...
          function (err, rows) {
            ...
            connection.query(
              "insert into style (" +
                ...
                ") values (" +
                `"` +
                Object.values(data).toString().replaceAll(",", `","`) +
                `"` +
                ")",
              [],
              function (err, rows) {
...
app.post("/dinnerdelete", function (req, res) {
  ...
  connection.query(
    "select * from dinner where dinnerID=?",
    [dinnerID],
    function (err, rows) {
      if (rows.length) {
        connection.query(
          "delete from dinner where dinnerID=?",
          [dinnerID],
          function (err, rows) {
            //if(err) throw err;
...
app.post("/dinneradd", function (req, res) {
  ...
  connection.query(
    "select max(dinnerID) as max_ID from dinner",
    [],
    function (err, rows) {
      //if(err) throw err;
      k = rows[0].max_ID;
      ...
      connection.query(
        "insert into dinner (dinnerID) values (?)",
        [k + 1],
        function (err, rows) {
          //if(err) throw err;
          res.json(rows);
...
app.post("/voucheradd", function (req, res) {
  ...
  connection.query(
    "select max(voucherID) as max_ID from voucher",
    [],
    function (err, rows) {
      //if(err) throw err;
      k = rows[0].max_ID;
      ...
      connection.query(
        "insert into voucher (voucherID) values (?)",
        [k + 1],
        function (err, rows) {
          //if(err) throw err;
          res.json(rows);
...
app.post("/orderpending", function (req, res) {
  connection.query(
    "select * from ordering order by orderID desc",
    [],
    function (err, rows) {
...
app.post("/orderalter", function (req, res) {
  ...
  connection.query(
    ...
    function (err, rows) {
      //if(err) throw err;
      connection.query(
        "update ordering set description=? where orderID=?",
        [state, orderID],
        function (err, rows) {
          //if(err) throw err;
...
app.post("/eventadd", function (req, res) {
  ...
  connection.query(
    ...
    function (err, rows) {
      ...
      connection.query(
        "insert into event (eventID) values (?)",
        [n + 1],
        function (err, rows) {
          //if(err) throw err;
          res.json(rows);
